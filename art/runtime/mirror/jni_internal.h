...
ALWAYS_INLINE
static bool IsIndexId(jmethodID mid) { // TODO: ing
...
template <bool kEnableIndexIds = true>
ALWAYS_INLINE
static inline jmethodID EncodeArtMethod(ArtMethod* art_method) REQUIRES_SHARED(Locks::mutator_lock_) { // TODO: ing
  if (kEnableIndexIds && Runtime::Current()->GetJniIdType() != JniIdType::kPointer) {
    return Runtime::Current()->GetJniIdManager()->EncodeMethodId(art_method);
  } else {
...
template <bool kEnableIndexIds = true>
ALWAYS_INLINE
static inline ArtMethod* DecodeArtMethod(jmethodID method_id) { // TODO: ing
